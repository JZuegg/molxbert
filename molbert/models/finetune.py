...
from torchmetrics import MeanSquaredError, MeanSquaredError, AUROC, AveragePrecision, Accuracy, MeanAbsoluteError, R2Score
    ...
    def evaluate_metrics(self, batch_labels, batch_predictions) -> Dict[str, torch.Tensor]:
        ...
        if self.hparams.mode == 'classification':
            ...
            metrics = {
                ...
                'RMSE': lambda: MeanSquaredError()(preds, batch_labels),
                'MSE': lambda: MeanSquaredError()(preds, batch_labels),
                'R2' : lambda: R2Score()(preds, batch_labels),
                # sklearn metrics work the other way round metric_fn(y_true, y_pred)
                #'R2': lambda: R2Score(batch_labels.cpu(), preds.cpu()),
            }
